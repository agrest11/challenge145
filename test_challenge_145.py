...
class Config:
    ...
    SOLUTION_SRC_FILE_NAME = "main.py"
    ...
    # Maximum length of line to print for failed cases
    # Set to -1 to print full line
    TRUNCATE_FAILED_CASES = 1000
...
def print_extra_stats(test_inp: TestInp, test_out: TestOut, num_cases: int) -> None:
    max_n = max(int(x[0].split()[0]) for x in test_inp)
    avg_n = sum(int(x[0].split()[0]) for x in test_inp) // num_cases
    max_q = max(int(x[0].split()[1]) for x in test_inp)
    avg_q = sum(int(x[0].split()[1]) for x in test_inp) // num_cases

    print(f" - Max N: {yellow}{max_n:_}{reset}")
    print(f" - Average N: {yellow}{avg_n:_}{reset}")
    print(f" - Max Q: {yellow}{max_q:_}{reset}")
    print(f" - Average Q: {yellow}{avg_q:_}{reset}")
...
def check_result(
    ...
    passed = i = oi = 0
    max_line_len = Config.TRUNCATE_FAILED_CASES if Config.TRUNCATE_FAILED_CASES > 0 else 10**6
    for i, (inp, exp) in enumerate(
        ...
        if out != exp:
            if i - passed <= Config.NUM_FAILED or Config.NUM_FAILED == -1:
                print(f"Test nr:{i}\n      Input: {cyan}")
                for line in inp:
                    print(line[:max_line_len] + " ..." if len(line) > max_line_len else line)
                print(
                    f"{reset}Your output: {find_diff(out, exp) if out else f'{red_bg}None{reset}'}"
                )
...
def parse_args() -> None:
    ...
    parser.add_argument(
        ...
        metavar="<int>",
        type=int,
        help="Maximum line length to print failed case input (-1 to print full line).",
    )
    parser.add_argument(
        "-l",
        "--nolength",
    ...
    if args.truncate:
        Config.TRUNCATE_FAILED_CASES = args.truncate
    if args.nolength:
